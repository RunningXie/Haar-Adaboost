...
class Detector(object):
    ...
    def detectFace(self, fileName, _show=True, _save=False, _saveInfo=False):
        ...
        scaledWindows = []
        for scale in np.arange(self.DETECT_START, self.DETECT_END, self.DETECT_STEP):#np.arange创建数组，scale从DETECT_START到DETECT_END以DETECT_STEP的间距递增
    ...
    def _detectScaledWindow(self, scaledWindows, img):
        ...
        for window in range(scaledWindows.shape[0]):#.shape[0]返回矩阵第一个维度的长度（行数）
