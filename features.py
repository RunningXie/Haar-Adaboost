...
def getFeatures():
    ...
    print(len(trainFaceFiles))
    print(len(trainNonFaceFiles))
    print(len(haar.features)+1)
    featureValMat = np.zeros((len(trainFaceFiles) + len(trainNonFaceFiles), len(haar.features)+1), dtype='float32')#数组太大的话会报错，memory error
